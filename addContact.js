...
function loadPage() {
    return fetch(`http://localhost/WebProject_2024/addContact.php?contact_id=${contactId}`)
     .then(response => {
         if(response.ok) {
 ...
 function updateContactType(action, contactId) {
     ...
     fetch(`http://localhost/WebProject_2024/addContact.php?contact_id=${contactId}&action=${action}`)
 ...
 function postNote(contactId) {
     ...
     fetch('http://localhost/WebProject_2024/addContact.php', {
         method: 'POST',
         mode: "cors",
         headers: {
